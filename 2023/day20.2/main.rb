...
mod = modules['rx']
while !mod.is_a?(Conjunction)
    raise 'multiple paths to `rx` are not supported' if mod.inputs.length > 1
    mod = mod.inputs.first
end

inputs = Hash[mod.state.collect {|i, s| [i, false]}]

pushes = 0
while inputs.value?(false)
    modules['button'].push
    pushes += 1
    processed = [modules['button']]
    while (processed = processed.collect(&:tick).compact.flatten).any?
        if mod.state.value?(CommunicationModule::HIGH)
            mod.state.each do |i, s|
                inputs[i] = pushes if !inputs[i] && s == CommunicationModule::HIGH
            end
        end
    end
end

puts "RX on after #{inputs.values.reduce { |acc, p| acc.lcm(p) }} button pushes!"