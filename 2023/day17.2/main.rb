...
def print_nodes(nodes)
    nodes.each do |node|
        puts "\t(#{node.cell.row}, #{node.cell.col}): #{node.step} -> #{direction_to_sym(node.direction)}"
    end
end

def print_neighbors(neighbors)
    neighbors.each do |node, neighbor_nodes|
        puts "(#{node.cell.row}, #{node.cell.col}): #{node.step} -> #{direction_to_sym(node.direction)} ($#{node.weight}):"
...
def nodes_for_cell(cell, nodes)
    nodes.select { |node| node.cell == cell }
end
...
class City < Matrix
    ...
    def cart_buildnodes(start=[0,0], turn_after = MUST_TURN_STEPS, straight_until = CANT_TURN_STEPS)
    ...
    def cart_pathtree(start=[0,0], turn_after = MUST_TURN_STEPS, straight_until = CANT_TURN_STEPS)
        distances = {}
        nodes, neighbors = cart_buildnodes(start, turn_after, straight_until)
